...
def test_parse_nulls(sdc_builder, sdc_executor, stage_attributes, mqtt_broker):
    """verify whether Mqtt subscriber origin can convert string constant to null.If we check 'parse_nulls'
    Mqtt subscriber should convert String mentioned in 'null_constant' to null."""
    data_topic = 'mqtt_subscriber_topic'
    INPUT_DATA = [['header1', 'header2', 'header3'], ['Field11', 'Field12', 'Field13'], ['Field21', 'Field22',
                                                                                         'Field23']]
    ...
    try:
        ...
        stage_attributes.update(dict(topic_filter=[data_topic]))
        stage_attributes.update({'header_line': 'WITH_HEADER'})
        if stage_attributes['parse_nulls']:
            stage_attributes.update({'null_constant': 'Field21'})
        mqtt_source.set_attributes(**stage_attributes)
        trash = pipeline_builder.add_stage('Trash')
        mqtt_source >> trash
        pipeline = pipeline_builder.build().configure_for_environment(mqtt_broker)
        sdc_executor.add_pipeline(pipeline)
        expected_message = INPUT_DATA_STR
        ...
        # It takes a bit of time for the pipeline to ACTUALLY start listening on the MQTT port, so if we don't sleep
        # here, the messages won't be delivered (without setting persist)
        time.sleep(1)
        records = [record.field for record in snapshot[mqtt_source.instance_name].output]
        if stage_attributes['parse_nulls']:
            assert records[1] == OrderedDict([('header1', None), ('header2', 'Field22'), ('header3', 'Field23')])
        else:
            assert records[1] == OrderedDict([('header1', 'Field21'), ('header2', 'Field22'),
                                              ('header3', 'Field23')])
    finally:
